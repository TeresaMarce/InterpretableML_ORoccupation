...
from sklearn.ensemble import GradientBoostingRegressor, GradientBoostingClassifier, RandomForestRegressor, RandomForestClassifier, BaggingRegressor, ExtraTreesRegressor
...
class RuleFit(BaseEstimator, TransformerMixin):
    ...
    def __init__(
            ...
            memory_par=0.01,
            tree_generator=None,
            regularization=None, #added to choose regularization type
            rfmode='regress',
        ...
        self.cv=cv
        self.tol=tol
        self.regularization=regularization #added to choose regularization type
        self.beta=beta
        self.alpha=alpha
    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if 'r' in self.model_type:
            ...
            if   self.rfmode=='regress':
                if type(self.tree_generator) not in [GradientBoostingRegressor, RandomForestRegressor, BaggingRegressor, ExtraTreesRegressor]:
                    raise ValueError("RuleFit only works with RandomForest, BoostingRegressor, BaggingRegressor and ExtraTreesRegressor")
            else:
            ...
            if not self.exp_rand_tree_size: # simply fit with constant tree size
                ...
                for i_size in np.arange(len(tree_sizes)):
                    ...
                    if isinstance(self.tree_generator, RandomForestRegressor) or isinstance(self.tree_generator, GradientBoostingRegressor) or isinstance(self.tree_generator, ExtraTreesRegressor):
                        
                        self.tree_generator.set_params(max_leaf_nodes=size)
                    else:
                        self.tree_generator.set_params(base_estimator__max_leaf_nodes=size) #added by Teresa beacause 
            ...
            if isinstance(self.tree_generator, RandomForestRegressor) or isinstance(self.tree_generator, BaggingRegressor) or isinstance(self.tree_generator, ExtraTreesRegressor):
        ...
        if self.rfmode=='regress':
            ...
            if self.regularization is None or self.regularization=='LassoCV':
                ...
                self.regularizationcv = LassoCV(
                    n_alphas=n_alphas, alphas=alphas, cv=self.cv,
                    max_iter=self.max_iter, tol=self.tol,
                    n_jobs=self.n_jobs,
                    random_state=self.random_state)
                self.regularizationcv.fit(X_concat, y)
                self.coef_=self.regularizationcv.coef_
                self.intercept_=self.regularizationcv.intercept_
            ...
            if self.regularization=='ElasticNetCV':
                ...
                self.regularizationcv = ElasticNetCV(
                    n_alphas=n_alphas, alphas=alphas, cv=self.cv,
                    max_iter=self.max_iter, tol=self.tol,
                    n_jobs=self.n_jobs,
                    random_state=self.random_state)
            ...
            if self.regularization=='Pyowl':
                ...
                self.regularizationcv = OwlRegressor(weights=(alpha, beta))
                self.regularizationcv.fit(X_concat, y)
                self.coef_=self.regularizationcv.coef_ 
                #self.intercept_=self.regularizationcv.intercept_
    ...
    def predict_proba(self, X):
        ...
        if 'r' in self.model_type:
            ...
            if len(rule_coefs)>0:
                ...
                if X_rules.shape[0] >0:
                    X_concat = np.concatenate((X_concat, X_rules), axis=1)
        return self.regularizationcv.predict_proba(X_concat)
